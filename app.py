...
LE_PKL = os.path.join(DATA_DIR, 'label_encoder.p') # This will now store le.classes_
...
# --- FIX: Use a session state variable to track the current mode ---
...
# --- Mode: Collect Data ---
    ...
    if not name:
        ...
        # --- FIX: Use a button to start capture and reset count ---
...
# --- Mode: Recognize & Mark Attendance ---
    ...
    try:
        ...
        if os.path.exists(LE_PKL):
            ...
            label_encoder = LabelEncoder() # Create a new, empty encoder
            label_encoder.classes_ = saved_classes # Set its classes from the loaded file
    ...
    # --- FIX: Put Result Queue in session_state ---
    ...
    if model and label_encoder:
        ...
        webrtc_ctx = webrtc_streamer(
            key="recognize",
            mode=WebRtcMode.SENDRECV,
            # --- FIX: Pass the persistent queue from session_state ---
